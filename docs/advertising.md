...
3. Configure and publish a Google-certified CMP, such as Google's Privacy & messaging European regulations message, for the relevant site and regions. The `/rules` ad unit and `/privacy` page now include the required privacy choices reopening trigger (`googlefc.showRevocationMessage`). Test actual consent behaviour and revocation; this code does not implement a home-made consent banner.
