...
3. Configure and publish a Google-certified CMP, such as Google's Privacy & messaging European regulations message, for the relevant site and regions. The `/rules` ad unit includes a queued privacy-choices reopening trigger; `/privacy` links to it. A live CMP must still be configured and verified. Test actual consent behaviour and revocation; this code does not implement a home-made consent banner.
4. `/privacy` is configured with operator contact (Adeticket Inc., `adeticket@gmail.com`), session/cookie retention schedules, and advertising isolation disclosures.
